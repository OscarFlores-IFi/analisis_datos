...
for i in range(100):
    ...
    decision = decision[0]-decision[3] ## En el caso de muestra decision[0] era la probabilidad de que el siguiente patron fuera de subida, decision[1] era la probabilidad de que el siguiente patrón fuera de bajada. Por ello se restan.
    print(decision)
    
#    if i%2 == 0:
#        situacion = compra(situacion[0], situacion[1],close[i],np.ceil(np.random.randint(0,dinero//close[i])),comision, historial)
#    if i%2 == 1:
#        situacion = venta(situacion[0], situacion[1],close[i],np.ceil(np.random.randint(0,situacion[0])),comision, historial)
print(situacion)
